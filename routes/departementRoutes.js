...
// Handler functions
const handleGetAll = (_req, res, next) => {
...
router.get('/details', validateDepartement, cacheMiddleware((req) => `dept_details_${req.query.dept}`), handleGetDetails);
...
router.get('/names', validateDepartement, cacheMiddleware((req) => `dept_names_${req.query.dept}`), handleGetNames);
...
router.get('/names_history', validateDepartement, cacheMiddleware((req) => `dept_names_history_${req.query.dept}`), handleGetNamesHistory);
...
router.get('/crime', validateDepartement, cacheMiddleware((req) => `dept_crime_${req.query.dept}`), handleGetCrime);
...
router.get('/crime_history', validateDepartement, cacheMiddleware((req) => `dept_crime_history_${req.query.dept}`), handleGetCrimeHistory);

const handleGetPrefet = (req, res, next) => {
  const handleDbError = createDbHandler(res, next);
  const { dept } = req.query;
...
router.get('/prefet', validateDepartement, cacheMiddleware((req) => `prefet_${req.query.dept}`), handleGetPrefet);

module.exports = { handleGetAll, handleGetDetails, handleGetNames, handleGetNamesHistory, handleGetCrime, handleGetCrimeHistory, handleGetPrefet, router };